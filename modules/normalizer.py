...
MATERIAL_MAP = {
    ...
    "gold": "金",
    # 銀
    # クライアント要望（2026-08・3000921）: 「銀」は色と誤認されるため、
    # 925刻印があるものは「シルバー925 SV925」で出力する
    "sv925": "シルバー925 SV925",
    "925": "シルバー925 SV925",
    "sterling silver": "シルバー925 SV925",
    "シルバー925": "シルバー925 SV925",
    "silver": "銀",
    "ag": "銀",
    # セラミック
    "ceramic": "セラミック",
...
def normalize_material(material: str) -> str:
    ...
    クライアント指示（2026-08）により「金」「銀」「樹脂」は最終出力を無効化する
    （実例 3000658: 裏蓋刻印は「WGP / CITIZEN / 6700 / 4-67370IS / 5080609」
    〔WGP=White Gold Plated〕で「金」の刻印は無いのに素材「金」が出力された。
    実例 3000921: 925刻印で「銀」と出力され、色と誤認される恐れがあると指摘。
    質感推測・色誤認による誤りを避けるための無効化で、該当素材の商品でも
    空欄になることはクライアント合意済み）。
    「18K」「K18」「シルバー925 SV925」「GP」等の刻印ベースの具体的な
    素材表記は対象外のまま維持する。
    """
    if not material:
        return ""
